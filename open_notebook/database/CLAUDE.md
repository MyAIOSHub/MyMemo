...
- `AsyncMigrationManager`: Main orchestrator
  - Loads 16 up migrations + 16 down migrations (hard-coded in __init__; migrations 11-12 add credential system, 13 adds model-credential link, 14 adds podcast model registry fields, 15 adds the `memory_ref` column on `source`, 16 is now a deprecated no-op — see migrations/16.surrealql)
...
- **Hard-coded migration files**: AsyncMigrationManager lists migrations 1-16 explicitly; adding new migration requires code change (not auto-discovery)
